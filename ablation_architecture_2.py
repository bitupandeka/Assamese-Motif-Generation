...
def DilatedConvBlock(x, width):
    conv1 = layers.Conv2D(width // 3, 3, padding='same', dilation_rate=1)(x)
    conv2 = layers.Conv2D(width // 3, 3, padding='same', dilation_rate=2)(x)
    conv3 = layers.Conv2D(width // 3, 3, padding='same', dilation_rate=4)(x)
...
def get_network(image_size, widths, block_depth):
    ...
    # Bottleneck with DilatedConvBlock
    x_left = DilatedConvBlock(x_left, widths[-1])
    x_bottom = DilatedConvBlock(x_bottom, widths[-1])
    ...
    return keras.Model([input_image, noise_variances], output, name="arch_2")
